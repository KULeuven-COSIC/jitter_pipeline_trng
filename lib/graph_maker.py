    ...
    def create_ax(self, x_slice: Union[slice, int]=slice(None),
        ...
        if y_label_color is not None:
            if isinstance(y_label_color, int):
                gp_dat_colors: List[str] = self._graph_params['data']['cols'] # type: ignore
                y_color = gp_dat_colors[y_label_color]
            else:
                gp_dat_named_colors: Dict[str, str] = self._graph_params['data']['named_cols'] # type: ignore # pylint: disable=line-too-long

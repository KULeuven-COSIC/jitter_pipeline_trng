"""Pipelined TRNG stochasic model script."""
...
# else:
#     test = int(sys.argv[1])
#     print('Test selected: ' + str(test))

# if (test != None):
#     jitStrength = 150e-6 # 72e-6 #ns

#     mu_dc_0         = 2*np.pi/3.2
#     mu_dc_1         = 2*np.pi/3.14
#     sigma_dc_0     = np.sqrt(jitStrength)*mu_dc_0 #0.1/6
#     sigma_dc_1     = np.sqrt(jitStrength)*mu_dc_1 #0.1/6

#     mu_vdl_0         = 2*np.pi/(0.5+0.0015/2)
#     mu_vdl_1         = 2*np.pi/(0.5-0.0015/2)
#     sigma_vdl_0     = np.sqrt(jitStrength)*mu_vdl_0 #0.1
#     sigma_vdl_1     = np.sqrt(jitStrength)*mu_vdl_1 #0.1
...
#     model_data = ModelData(mu_dc_0, mu_dc_1, sigma_dc_0, sigma_dc_1, mu_vdl_0, mu_vdl_1, sigma_vdl_0, sigma_vdl_1, n)
#     approx = Approx(500)
#     model = Model(model_data, approx)

# if (test == 0):        #Testing f_cdf_td and f_pdf_td
#     FTD_slow = [0]*200
#     fTD_slow = [0]*200
#     xTD_slow = [0]*200
#     print('slow')
...
#         xTD_slow[i] = xi
#         itStart = time.time()
#         FTD_slow[i] = model.f_cdf_td(xi, False, False)
#         itMid = time.time()
#         fTD_slow[i] = model.f_pdf_td(xi, False, False)
#         itEnd = time.time()
#         print('Iteration time: ' + str(itMid-itStart) + ' ' + str(itEnd-itMid))
#     timeTaken = time.time() - start_time
#     print('Slow elapsed: ' + str(timeTaken))
#     FTD_fast = [0]*200
#     fTD_fast = [0]*200
#     xTD_fast = [0]*200
#     print('fast')
...
#         xTD_fast[i] = xi
#         itStart = time.time()
#         FTD_fast[i] = model.f_cdf_td(xi, True, True)
#         itMid = time.time()
#         fTD_fast[i] = model.f_pdf_td(xi, True, True)
#         itEnd = time.time()
#         print('Iteration time: ' + str(itMid-itStart) + ' ' + str(itEnd-itMid))
#     timeTaken = time.time() - start_time
#     print('Fast elapsed: ' + str(timeTaken))

#     plt.subplot(211)
#     plt.plot(xTD_slow, FTD_slow)
#     plt.plot(xTD_fast, FTD_fast)
#     plt.subplot(212)
#     plt.plot(xTD_slow, fTD_slow)
#     plt.plot(xTD_fast, fTD_fast)
#     plt.show()

# elif (test == 1):        #Testing f_cdf_t_pi and f_pdf_t_pi
#     fTpi = [0]*2000
#     FTpi = [0]*2000
#     xTpi = [0]*2000
#     for i in range(2000):
#         xi = (i-500)/400
#         xTpi[i] = xi
#         fTpi[i] = model.f_pdf_t_pi(xi, Phi)
#         FTpi[i] = model.f_cdf_t_pi(xi, Phi)
...
# elif (test == 2):        #Testing f_cdf_phid and f_pdf_phid
#     fPhiD = [0]*200
#     FPhiD = [0]*200
#     xPhiD = [0]*200
#     for i in range(200):
#         xi = (i-100)/10
#         xPhiD[i] = xi
#         fPhiD[i] = model.f_pdf_phid(xi, t)
#         FPhiD[i] = model.f_cdf_phid(xi, t)

#     plt.subplot(211)
#     plt.plot(xPhiD, FPhiD)
#     plt.subplot(212)
#     plt.plot(xPhiD, fPhiD)
#     plt.show()

# elif (test == 3):        #Testing f_cdf_rc and f_pdf_rc
#     fRC = [0]*200
#     FRC = [0]*200
#     xRC = [0]*200
#     for i in range(200):
#         xRC[i] = i
#         FRC[i] = model.f_cdf_rc(i, t*mu_vdl_1, t)
#         fRC[i] = model.f_pdf_rc(i, t*mu_vdl_1, t)
...
# elif (test == 4):        #Testing f_pdf_rj
#     fRJ = [0]*200
#     xRJ = [0]*200
#     for i in range(200):
#         xRJ[i] = i
#         fRJ[i] = model.f_pdf_rj(i, t*mu_vdl_1, t, False, True)

#     plt.plot(xRJ, fRJ)
#     plt.show()

# elif (test == 5):        #Testing f_pdf_r
#     fR = [0]*200
#     xR = [0]*200
#     for i in range(200):
#         print(i)
#         xR[i] = i
#         fR[i] = model.f_pdf_r(i, False, True)

#     plt.plot(xR, fR)
#     plt.show()

# elif (test == 6):        #Testing parallel f_pdf_r
#     result = model.iteratef_R1(0, 200, False, True)
...
# elif (test == 8):
#     start_time = time.time()
#     model.f_pdf_rj(20, mu_vdl_0*0.01, 0.01, True, True)
#     end_time = time.time()
#     print('Initial evaluation time: ' + str(end_time-start_time))
#     start_time = time.time()
#     model.f_pdf_rj(20, mu_vdl_0*0.01, -0.01, True, True)
#     end_time = time.time()
...
# elif (test == 10):        #Entropy vs DeltaVDL test
#     #Ds = [0.001*np.sqrt(2)**x for x in range(18)]
#     Ds = [0.004 + 0.004*x for x in range(100)]
#     Hs = [0]*(len(Ds)*2)
#     xs = [0]*(len(Ds)*2)
...
#             PVDL0 = 0.5 - D/2
#             PVDL1 = 0.5 + D/2
#             PDC1 = PVDL0*PVDL1/abs(D)
#             PDC0 = PDC1 - 0.125

#             mu_dc_0         = 2*np.pi/PDC0
#             mu_dc_1         = 2*np.pi/PDC1
#             sigma_dc_0     = np.sqrt(jitStrength)*mu_dc_0
#             sigma_dc_1     = np.sqrt(jitStrength)*mu_dc_1

#             mu_vdl_0         = 2*np.pi/PVDL0
#             mu_vdl_1         = 2*np.pi/PVDL1
#             sigma_vdl_0     = np.sqrt(jitStrength)*mu_vdl_0
#             sigma_vdl_1     = np.sqrt(jitStrength)*mu_vdl_1

#             model_data = ModelData(mu_dc_0, mu_dc_1, sigma_dc_0, sigma_dc_1, mu_vdl_0, mu_vdl_1, sigma_vdl_0, sigma_vdl_1, n)
#             approx = Approx(500)
#             model = Model(model_data, approx, 'HvsDeltaVDL.csv')

#             model.full_describe(200)
#             P_b = model.p_b1()
#             xs[2*i+j] = D
#             if ((P_b == 0) | (P_b == 1)):
#                 Hs[2*i+j] = 0
#             else:
#                 Hs[2*i+j] = -P_b*np.log2(P_b) - (1-P_b)*np.log2(1-P_b)
#     plt.plot(xs, Hs, 'o')
#     plt.show()

# elif (test == 11):        #Sweep DeltaVDL for different jitterstrength
#     Ds = [10**(-3) * 1.125336**x for x in range(40)]
#     jits = [5e-6 * 2**x for x in range(10)]
#     for ji in range(len(jits)):
...
#                 mu_dc_0         = 2*np.pi/PDC0
#                 mu_dc_1         = 2*np.pi/PDC1
#                 sigma_dc_0     = np.sqrt(jits[ji])*mu_dc_0
#                 sigma_dc_1     = np.sqrt(jits[ji])*mu_dc_1

#                 mu_vdl_0         = 2*np.pi/PVDL0
#                 mu_vdl_1         = 2*np.pi/PVDL1
#                 sigma_vdl_0     = np.sqrt(jits[ji])*mu_vdl_0
#                 sigma_vdl_1     = np.sqrt(jits[ji])*mu_vdl_1

#                 model_data = ModelData(mu_dc_0, mu_dc_1, sigma_dc_0, sigma_dc_1, mu_vdl_0, mu_vdl_1, sigma_vdl_0, sigma_vdl_1, n)
#                 approx = Approx(500)
#                 model = Model(model_data, approx, 'jitVDLSweep.csv')
